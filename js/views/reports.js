...
function filterRow(dt, customerId, status, from, to, rowCustomerId, rowStatus){const t=dateToTimestamp(dt);if(!Number.isFinite(t))return false;if(from&&t<dateToTimestamp(from))return false;if(to&&t>dateToTimestamp(to)+86399999)return false;if(customerId&&String(rowCustomerId)!==String(customerId))return false;if(status&&rowStatus!==status)return false;return true;}
const reportDefs=[
...
export async function renderReports(App, params={}){
  ...
  function invoicesReport(f){const rows=invoices.filter(i=>filterRow(i.date||i.createdAt,f.customerId,f.status,f.from,f.to,i.customerId,i.status)).sort((a,b)=>dateToTimestamp(b.date||b.createdAt)-dateToTimestamp(a.date||a.createdAt));document.getElementById('filtered-out').innerHTML=summaryCards([['تعداد',num(rows.length)],['مجموع',money(rows.reduce((s,i)=>s+(i.total||0),0))]])+`<div class="report-list">${rows.map(invoiceRow).join('')||'<div class="empty">فاکتوری یافت نشد.</div>'}</div>`;bindInvoiceRows();}
