...
import { INVOICE_STATUS, validateInvoice, parseNormalizedNumber } from '../validators.js';
...
export async function renderInvoices(App, opts = {}) {
  ...
  function form(preselected = null, existing = null) {
    ...
    const addRow = (item=null) => {
     ...
     const readItems=()=>[...itemsEl.querySelectorAll('.invoice-row')].map(row=>{const it=row._item||{};const manual=!row._item;return {type:manual?'manual':(it.type||'product'),itemId:it.id??it.itemId??null,name:row.querySelector('.item-search').value.trim(),unit:it.unit||'مورد',quantity:parseNormalizedNumber(row.querySelector('.qty').value),unitPrice:parseNormalizedNumber(row.querySelector('.price').value),discountType:row.querySelector('.rdtype')?.value||it.discountType||'amount',discountInput:parseNormalizedNumber(row.querySelector('.rdisc').value),discount:parseNormalizedNumber(row.querySelector('.rdisc').value)};});
    const refreshTotals=()=>{
      const its=readItems();
       const d=parseNormalizedNumber(f.discount.value);
       const extra=parseNormalizedNumber(f.extraCosts?.value);
...
export async function renderInvoiceDetail(App, params) {
  ...
  const storedTotal=Number.isFinite(Number(invoice.total)) ? Number(invoice.total) : computed.total;
   const effectivePaid=Math.max(0,Math.round(invoicePaidAmount(invoice,transactions)));
